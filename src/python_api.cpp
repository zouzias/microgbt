...
PYBIND11_MODULE(microgbtpy, m) {
  ...
  gbt.def("train", &microgbt::GBT::trainPython,
          ...
          py::call_guard<py::gil_scoped_release>(), pybind11::arg("train_X"),
          pybind11::arg("train_y"), pybind11::arg("valid_x"),
          pybind11::arg("valid_y"), pybind11::arg("num_iterations"),
          pybind11::arg("early_stopping_rounds") = 5);

  // Predict API
  gbt.def("predict", &microgbt::GBT::predict,
          "Python API to get predictions using microGBT", pybind11::arg("x"),
          pybind11::arg("num_iterations") = 0);

  gbt.def("__repr__", [](const microgbt::GBT &a) {
    std::string repr;
    repr += "<microgbt>[";
    repr += "learningRate:";
