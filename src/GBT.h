...
class GBT {
  ...
  double _lambda, _gamma, _minSplitGain, _learningRate, _minTreeSize,
      _shrinkageRate;
  ...
  /**
   * Return a single decision/regression tree given training data, gradient,
   * hessian vectors and shrinkage rate
   *
   * @param trainSet Input train dataset (features matrix and target vector)
   * @param gradient Gradient vector: each coordinate corresponds to sample (row
   * index)
   * @param hessian Hessian vector: each coordinate corresponds to sample (row
   * index)
   * @param shrinkageRate Shrinkage rate
   */
  Tree buildTree(const Dataset &trainSet, const Vector &previousPreds,
 ...
 public:
   ...
   * @param earlyStoppingRounds number of rounds to consider for early stopping,
  ...
  void train(const Dataset &trainSet, const Dataset &validSet,
    ...
    double learningRate = _shrinkageRate,
           bestValidationLoss = std::numeric_limits<double>::max();
   ...
   * @param numIterations Number of iterations to use for prediction. This is
   * used in case that early stopping took place
