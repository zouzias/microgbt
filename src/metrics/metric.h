...
class Metric {
 public:
   ...
   */
  virtual Vector gradients(const Vector &predictions,
                           const Vector &targets) const = 0;
   ...
   * @return Loss value
   */
  virtual double lossAt(const Vector &predictions,
                        const Vector &targets) const = 0;

  /**
   * Transformation required from Gradient Boosting Trees (GBT) scores to
   * prediction.
   *
   * For example, the logloss will transform the score using the logit function
   * 1 / (1 + exp(-score))
   *
