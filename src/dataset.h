...
/**
 * Dataset represents a machine learning "design matrix" and target vector, (X,
 * y) where the rows and columns of matrix X represent the samples and features,
 * respectively. y is the target vector to be predicted
 */
class Dataset {
  // Design matrix, each row corresponds to a sample; each column corresponds to
  // a feature
 ...
 public:
  ...
  Dataset(const MatrixType &X, const Vector &y)
      : _sortedMatrixIdx(X.rows(), X.cols()), _rowIndices(y.size()) {
   ...
   * @param side
   */
  Dataset(Dataset const &dataset, const SplitInfo &bestGain,
          SplitInfo::Side side) {
    _X = dataset.X();
    _y = dataset.yptr();
  ...
  inline long nRows() const {
    return static_cast<long>(this->_rowIndices.size());
  }
   ...
   *
   * It returns sorted indices depending on type of feature (categorical or
   * numeric): Categorical feature: performs mean target encoding (see
   * feature/categorical branch) Numerical feature: natural sort on numeric
   * value
   *
