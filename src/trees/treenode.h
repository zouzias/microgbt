...
class TreeNode {
 ...
 public:
  explicit TreeNode(double lambda, double minSplitGain, double minTreeSize,
                    int maxDepth) {
    _lambda = lambda;
   ...
   * 3) Use a linear scan to decide the best split along that feature (if
   * categorical perform Mean Target Encoding) 4) Take the best split solution
   * (that maximises gain reduction) over all features 5) Recurse on the left
   * and right side of the best split
   *
  ...
  void build(const Dataset &trainSet, const Vector &previousPreds,
    ...
    std::unique_ptr<microgbt::Splitter> splitter =
        make_unique<microgbt::NumericalSplitter>(_lambda);
    ...
    Vector leftPreviousPreds =
        bestGain.split(previousPreds, SplitInfo::Side::Left);
    this->leftSubTree = std::unique_ptr<TreeNode>(
        new TreeNode(_lambda, _minSplitGain, _minTreeSize, _maxDepth));
    leftSubTree->build(leftDataset, leftPreviousPreds, leftGradient,
                       leftHessian, shrinkage, depth + 1);
    ...
    Vector rightPreviousPreds =
        bestGain.split(previousPreds, SplitInfo::Side::Right);

    this->rightSubTree = std::unique_ptr<TreeNode>(
        new TreeNode(_lambda, _minSplitGain, _minTreeSize, _maxDepth));
    rightSubTree->build(rightDataset, rightPreviousPreds, rightGradient,
                        rightHessian, shrinkage, depth + 1);
  }
