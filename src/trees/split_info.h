...
class SplitInfo {
  ...
  double _bestGain = std::numeric_limits<double>::min(),
         _bestSplitNumericValue = 0.0;
 ...
 public:
  ...
  SplitInfo(const Eigen::RowVectorXi &sortedFeatureIndices, double gain,
            double bestSplitNumericValue, size_t bestSortedIdx)
  ...
  bool operator<(const SplitInfo &rhs) const {
    return this->_bestGain <= rhs.bestGain();
  }
   ...
   *
   * SplitInfo has the left and right subset of indices corresponding to the
  ...
  VectorD split(const VectorD &vector, const SplitInfo::Side &side) const {
    ...
    std::transform(rowIndices.begin(), rowIndices.end(),
                   ...
                   [&vector](size_t rowIndex) { return vector[rowIndex]; });
