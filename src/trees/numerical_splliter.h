...
class NumericalSplitter : public Splitter {
   ...
   */
  constexpr double calc_split_gain(double G, double H, double G_l,
                                   double H_l) const {
    return objective(G_l, H_l) + objective(G - G_l, H - H_l) -
           objective(G, H) / 2.0;  // TODO: minus \gamma
  }
   ...
   */
  SplitInfo optimumGainByFeature(const Dataset &dataset, const Vector &gradient,
                                 const Vector &hessian, long featureId) const {
    // Sort the feature by value and return permutation of indices (i.e.,
    // argsort)
    const Eigen::RowVectorXi &sortedInstanceIds =
        dataset.sortedColumnIndices(featureId);
    ...
    // For each feature, compute split gain and keep the split index with
    // maximum gain
    Vector gainPerOrderedSampleIndex(dataset.nRows());
    for (long i = 0; i < dataset.nRows(); i++) {
      gainPerOrderedSampleIndex[i] =
          calc_split_gain(cum_sum_g, cum_sum_h, cum_sum_G[i], cum_sum_H[i]);
    }

    long bestGainIndex = std::max_element(gainPerOrderedSampleIndex.begin(),
                                          gainPerOrderedSampleIndex.end()) -
                         gainPerOrderedSampleIndex.begin();
    double bestGain = gainPerOrderedSampleIndex[bestGainIndex];
    double bestSplitNumericValue =
        dataset.row(sortedInstanceIds[bestGainIndex])[featureId];
    long bestSortedIndex = bestGainIndex + 1;

    return SplitInfo(sortedInstanceIds, bestGain, bestSplitNumericValue,
                     bestSortedIndex);
  }
 ...
 public:
  ...
  SplitInfo findBestSplit(const Dataset &trainSet, const Vector &gradient,
    ...
    for (long featureId = 0; featureId < numFeatures; featureId++) {
      gainPerFeature[featureId] =
          optimumGainByFeature(trainSet, gradient, hessian, featureId);
    }

    // 3) Use a linear scan to decide the best split along that feature
    // 4) Take the best split solution (that maximises gain reduction) over all
    // features
    long bestFeatureId =
        std::max_element(gainPerFeature.begin(), gainPerFeature.end()) -
        gainPerFeature.begin();
